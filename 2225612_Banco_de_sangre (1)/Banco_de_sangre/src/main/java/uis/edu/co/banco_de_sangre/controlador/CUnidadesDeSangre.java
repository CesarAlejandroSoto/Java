...
try {
        ...
        String sql = "SELECT us.id_unidades_sangre, don.nombre, don.apellido, tcs.nombre_tipo_componente, eu.estado, us.fecha_procesamiento " +
                     ...
                     "JOIN tipo_componente_sanguineo tcs ON us.tipo_componente_sanguineo_id = tcs.id_tipo_componente " +
                     "JOIN estado_unidades eu ON us.estado_unidades_id = eu.id_estado_unidades " +
                     "WHERE eu.estado = 'Disponible' " + // Seleccionar solo unidades con estado "Disponible"
                     "ORDER BY us.id_unidades_sangre"; // Ordenar por ID de unidades de sangre
        PreparedStatement statement = connection.prepareStatement(sql);
        ResultSet resultSet = statement.executeQuery();
...
public void cargarDonantesEnTabla(JTable tabla) {
    ...
    try {
        // Crear una sentencia SQL para seleccionar todos los donantes
        ...
        // Iterar sobre los resultados y agregar cada donante a la tabla
        while (resultSet.next()) {
            ...
            // Aquí podrías obtener el nombre del grupo sanguíneo y formatear las fechas si es necesario
            
            // Agregar una fila con los datos del donante a la tabla
...
public void cargarUnidadesEnProcesoDeAnalisisYDistribucion(JTable tblUnidades) {
    try {
        ...
        // Consulta para obtener las unidades en proceso de análisis y distribución
        String sql = "SELECT us.id_unidades_sangre, don.nombre, don.apellido, tcs.nombre_tipo_componente, eu.estado, us.fecha_procesamiento " +
                     "FROM unidades_de_sangre us " +
                     "JOIN donantes don ON us.donacion = don.id_donante " +
                     "JOIN tipo_componente_sanguineo tcs ON us.tipo_componente_sanguineo_id = tcs.id_tipo_componente " +
                     "JOIN estado_unidades eu ON us.estado_unidades_id = eu.id_estado_unidades " +
                     "WHERE eu.estado IN ('En proceso de análisis', 'En proceso de distribución') " + // Seleccionar unidades en proceso de análisis y distribución
                     "ORDER BY us.id_unidades_sangre"; // Ordenar por ID de unidades de sangre
        ...
        // Agregar las unidades en proceso de análisis y distribución a la tabla
        while (resultSet.next()) {
            int id = resultSet.getInt("id_unidades_sangre");
            String nombre = resultSet.getString("nombre");
            String apellido = resultSet.getString("apellido");
